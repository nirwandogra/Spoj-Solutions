...
int backtrack(int day,int last,int used,int left,double checkval)
{
    if(checkval==0.00)
    {
        if(day!=K+1)
        {
            cout<<pos[pi(cost[last],val[last])]<<" ";
    ...
    for (int today=1; today<=N; today++)
    {
        ...
        else if(today==last)
        {
            ...
            if( (val[today]*factor)+dp[day+1][today][used+1][left-cost[today]]==checkval)
            {
                ...
                cout<<pos[pi(cost[today],val[today])]<<" ";
            ...
            for (int last=0; last<=N; last++)
            {
                for (int used=0; used<=3; used++)
                {
                    ...
                    for (int left=0; left<=M; left++)
                    {
                        ...
                        /// cout<<day<<" "<<last<<" "<<used<<" "<<left<<" "<<dp[day][last][used][left]<<endl;
                    }
                }
            }
        }
