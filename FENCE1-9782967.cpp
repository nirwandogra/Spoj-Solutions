...
ll int dp[76][12];ll int N;

ll int solve(ll int i,ll int mini)
{
  if(i==0) return 1;   
  if(dp[i][mini]!=0) return  dp[i][mini];   
  ll int ch=0;   
  for(ll j=mini;j<10;j++)
  {
       ch+=solve(i-1,max(mini,j));
  }
   return dp[i][mini]=ch;
}
//#define pie 3.14
