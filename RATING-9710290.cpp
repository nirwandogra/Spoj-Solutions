...
vector<pii>vec;
//int A[333333];int B[333333];
//typedef long long llong;
 
const int MAXN = 300000;
ll int tree[MAXN], A[MAXN], B[MAXN];
ll int N;ll int pos[MAXN];ll int out[MAXN];
ll int yy[MAXN]; 
int bit[100001];;
...
forr(i,N)
{
     //CO(vec[i].ff.ff<<" "<<vec[i].ff.ss)
}
     forr(i,N)
     {
          int newi=i;
          while(newi<N && vec[newi].ff.ff==vec[i].ff.ff && vec[newi].ff.ss==vec[i].ff.ss ) ++newi;
          forrr(j,i,newi)
          {out[pos[j]]=query(vec[i].ff.ss);}
          forrr(j,i,newi)
          {update(vec[i].ff.ss);}
          i=newi-1;
     }
