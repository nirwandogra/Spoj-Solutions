...
vector<pi>vec[111111];;;;queue<int>Q;;;;int from,to,dist,edge;;;;;
vector<pi>::iterator it;;;;;int start,N,EE,val,a,b,end;
int dp[111111];;;;
int dij()
{
...
//int st,end;
int T;C(T);while(T--)
{C(N>>EE>>start>>end) forr(i,N+100){vec[i].clear();}
forr(i,EE)
...
//C(start>>end);
dij();
if(dp[end]==INF){CO("NONE")continue;}
CO(dp[end])
}
return 0;
}
