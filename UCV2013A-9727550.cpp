...
long long powmod(ll n, ll p) {long long r = 1, b = n;while(p > 0) {if(p & 1) { r = r * b; if(r >= MOD) r %= MOD; }p >>= 1;b = b * b; if(b >= MOD) b %= MOD;}return r;}
...
int main()
{
 ...
 while(1)
 {
      ...
      if(N==0 && L==0)
      {return 0;}
      ll inv=powmod(N-1,MOD-2);
      ll val=(powmod(N,L+1)-1)%MOD;
      ll ans=((val*inv)%MOD-1)%MOD;
      ans=(ans+MOD)%MOD;
      co(ans)
 }
 double cl = clock();
